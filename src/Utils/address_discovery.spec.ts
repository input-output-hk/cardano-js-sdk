...
describe('addressDiscovery', () => {
  ...
  it('correctly returns address indexes and address type', () => {
    const internalAddresses = addressDiscoveryWithinBounds(RustCardano, { account, type: AddressType.internal, lowerBound: 0, upperBound: 19 })
    expect(internalAddresses[0].index).to.eql(0)
    expect(internalAddresses[0].type).to.eql(AddressType.internal)

    const externalAddresses = addressDiscoveryWithinBounds(RustCardano, { account, type: AddressType.external, lowerBound: 0, upperBound: 19 })
  ...
  describe('internal', () => {
    ...
    it('does not collide between different bounds', () => {
      const first20Addresses = addressDiscoveryWithinBounds(RustCardano, { account, type: AddressType.internal, lowerBound: 0, upperBound: 19 })
      const next20Addresses = addressDiscoveryWithinBounds(RustCardano, { account, type: AddressType.internal, lowerBound: 20, upperBound: 39 })
  ...
  describe('external', () => {
    ...
    it('does not collide between different bounds', () => {
      const first20Addresses = addressDiscoveryWithinBounds(RustCardano, { account, type: AddressType.external, lowerBound: 0, upperBound: 19 })
      const next20Addresses = addressDiscoveryWithinBounds(RustCardano, { account, type: AddressType.external, lowerBound: 20, upperBound: 39 })
      const addressSet = new Set(first20Addresses.concat(next20Addresses))
      expect([...addressSet].length).to.eql(40)
    })
  })

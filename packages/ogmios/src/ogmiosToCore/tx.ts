...
const mapBootstrapWitness = (b: Schema.BootstrapWitness): Cardano.BootstrapWitness => ({
  ...
  key: b.key ? Cardano.Ed25519PublicKey(b.key) : undefined,
  signature: b.signature ? Cardano.Ed25519Signature(b.signature) : undefined
});
...
const mapTxOut = (txOut: Schema.TxOut): Cardano.TxOut => ({
  ...
  // `txOut.datum` could be `[k: string]: unknown` as well
  datum: txOut.datum ? Cardano.util.Hash32ByteBase16(txOut.datum as Schema.Datum) : undefined,
  value: {
...
const mapValidityInterval = ({
  ...
  invalidBefore: invalidBefore ?? undefined,
  ...
  witness: {
    ...
    signatures: new Map(
      Object.entries(tx.witness.signatures).map(([key, value]) => [
        ...
        Cardano.Ed25519Signature(value)
      ])
    )
  }
});

...
describe('blockfrostWalletProvider', () => {
  describe('transactionsByHashes', () => {
    ...
    it.skip('has GenesisKeyDelegation cert', async () => {
      const [tx] = await walletProvider.transactionsByHashes([Cardano.TransactionId('someValue')]);
  ...
  describe('transactionsByAddresses', () => {
    it('Shelley address (addr_test1)', async () => {
      const txs = await walletProvider.transactionsByAddresses([
    ...
    it('returns transactions starting from sinceBlock param', async () => {
      ...
      const transactions = await walletProvider.transactionsByAddresses([address], 3_348_548);

...
import { CML } from '../CML';
import { ManagedFreeableScope, hexStringToBuffer } from '@cardano-sdk/util';
...
export const txOut = (scope: ManagedFreeableScope, core: Cardano.TxOut): TransactionOutput => {
  ...
  if (core.datum) {
    const cmlDatum = scope.manage(CML.PlutusData.from_bytes(hexStringToBuffer(core.datum)));
    cslTxOut.set_datum(scope.manage(Datum.new_data(cmlDatum)));
...
export const txWitnessRedeemers = (scope: ManagedFreeableScope, redeemers: Cardano.Redeemer[]): Redeemers => {
  ...
  for (const redeemer of redeemers) {
    ...
    const data = scope.manage(CML.PlutusData.from_bytes(hexStringToBuffer(redeemer.data)));
...
export const witnessSet = (scope: ManagedFreeableScope, witness: Cardano.Witness): TransactionWitnessSet => {
  ...
  if (witness.datums) {
    const list = scope.manage(CML.PlutusList.new());
    for (const datum of witness.datums) {
      list.add(scope.manage(CML.PlutusData.from_bytes(hexStringToBuffer(datum))));
    }
    txWitnessSet.set_plutus_data(list);
  }
  return txWitnessSet;
};

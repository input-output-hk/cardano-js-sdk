...
import { CborWriter } from './CBOR';
import { Decoder, Encoder } from 'borc';
...
export class Transaction {
  ...
  // This cache will be invalidated if any of the transaction properties changes after the object has be deserialized.
   ...
   */
  static async fromCbor(cbor: HexBlob): Promise<Transaction> {
    const txFrame = await Decoder.decodeFirst(Buffer.from(cbor, 'hex'));

    const body = CML.TransactionBody.from_bytes(Encoder.encode(txFrame[0]).valueOf());
    const witnessSet = CML.TransactionWitnessSet.from_bytes(Encoder.encode(txFrame[1]).valueOf());
    let isValid = true;
    let auxDataIndex;

    // The isValid flag was added in Alonzo era (onwards), mary era transactions only have three fields.
    if (txFrame.length === ALONZO_ERA_TX_FRAME_SIZE) {
      isValid = txFrame[2];
      auxDataIndex = 3;
    } else {
      auxDataIndex = 2;
    ...
    const auxData =
      txFrame[auxDataIndex] !== null ? CML.AuxiliaryData.from_bytes(Encoder.encode(txFrame[3]).valueOf()) : undefined;
   ...
   *
   * TODO: The scope parameters is needed while we remove the CML objects from the implementation. Once this is done this param can be removed.
   */
   ...
   */
  async clone(): Promise<Transaction> {

...
export class PlutusV4Script {
   ...
   * If you're creating this from those you should use PlutusV4Script.fromCbor() instead.
   */
  ...
  /**
   * Serializes a PlutusV4Script into CBOR format.
   *
   * @returns The PlutusV4Script in CBOR format.
   */
  toCbor(): HexBlob {
  ...
  /**
   * Deserializes the PlutusV4Script from a CBOR byte array.
   *
   * @param cbor The CBOR encoded PlutusV4Script object.
   * @returns The new PlutusV4Script instance.
   */
  static fromCbor(cbor: HexBlob): PlutusV4Script {
    const reader = new CborReader(cbor);

    const bytes = reader.readByteString();

    const script = new PlutusV4Script(HexBlob.fromBytes(bytes));
   ...
   *
   * @returns The Core PlutusScript object.
   */
  ...
  /**
   * Creates a PlutusV4Script object from the given Core PlutusScript object.
   *
   * @param plutusScript The core PlutusScript object.
   */
  static fromCore(plutusScript: Cardano.PlutusScript): PlutusV4Script {
    if (plutusScript.version !== PlutusLanguageVersion.V4)
      throw new InvalidArgumentError('script', 'Wrong plutus language version.');

    return new PlutusV4Script(plutusScript.bytes);
  }

  /**
   * Computes the script hash of this Plutus V4 script.
   *
  ...
  hash(): Crypto.Hash28ByteBase16 {
    ...
    // the script before hashing. The tag in the Dijkstra era for PlutusV4 is "\x04"
    const bytes = `04${this.rawBytes()}` as HexBlob;
   ...
   *
   * If you need "cborBytes" for cardano-cli use PlutusV4Script::toCbor() instead.
   *

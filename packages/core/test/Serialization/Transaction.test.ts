...
describe('Transaction', () => {
  it('round trip serializations produce the same CBOR output', async () => {
    const tx = await Transaction.fromCbor(HexBlob(TX));
    expect(tx.toCbor()).toBe(TX);
  });

  it('correctly deserialize a CBOR transaction', async () => {
    const tx = await Transaction.fromCbor(HexBlob(TX));
    expect(tx.toCore()).toEqual(babbageTx);
  });

  it('correctly converts from a Core transaction', async () => {
  ...
  it('correctly converts from a Babbage Core transaction', async () => {
    const scope = new ManagedFreeableScope();
    const tx = Transaction.fromCore(scope, babbageTx);
    expect(tx.toCore()).toEqual(babbageTx);
  });

  it('calling free several times doesnt throws an error', async () => {
    const tx = await Transaction.fromCbor(HexBlob(TX));
  ...
  it('can set the isValid fag on the transaction', async () => {
    const tx = await Transaction.fromCbor(HexBlob(TX));
    ...
    const tx2 = await Transaction.fromCbor(tx.toCbor());

    expect(tx2.isValid()).toEqual(false);
  });

  it('can set the txBody on the transaction', async () => {
    const scope = new ManagedFreeableScope();
    const tx = await Transaction.fromCbor(HexBlob(TX));
    const tx2 = await Transaction.fromCbor(HexBlob(TX2));

    tx.setBody(scope.manage(tx2.body()));

    // Perform a round trip serialization.
    const tx3 = await Transaction.fromCbor(tx.toCbor());

    expect(scope.manage(tx3.body()).to_bytes()).toEqual(scope.manage(tx2.body()).to_bytes());
  });

  it('can set the witness set on the transaction', async () => {
    const scope = new ManagedFreeableScope();
    const tx = await Transaction.fromCbor(HexBlob(TX));
    const tx2 = await Transaction.fromCbor(HexBlob(TX2));
  ...
  it('can perform a deep clone of the object', async () => {
    const scope = new ManagedFreeableScope();
    const referenceTx = await Transaction.fromCbor(HexBlob(TX));
    const tx = await Transaction.fromCbor(HexBlob(TX));
    const cloned = await tx.clone();
    const tx2 = await Transaction.fromCbor(HexBlob(TX2));
    ...
    const tx3 = await Transaction.fromCbor(cloned.toCbor());

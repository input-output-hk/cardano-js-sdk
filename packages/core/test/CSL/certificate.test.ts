...
describe('certificates', () => {
  ...
  describe('coreToCsl.certificate', () => {
    it('stakeKeyRegistration', () => {
      const certificate = coreToCsl.certificate.stakeKeyRegistration(scope, stakeKeyHash);
      ...
      const rewardAddress = scope.manage(CSL.RewardAddress.new(1, stakeCredential));
      const cslAddress = scope.manage(rewardAddress.to_address());
      expect(cslAddress.to_bech32()).toBe(rewardAccount);
    });

    it('stakeKeyDeregistration', () => {
      const certificate = coreToCsl.certificate.stakeKeyDeregistration(scope, stakeKeyHash);
    ...
    it('stakeDelegation', () => {
      const certificate = coreToCsl.certificate.stakeDelegation(scope, stakeKeyHash, poolId);
      ...
      const rewardAddress = scope.manage(CSL.RewardAddress.new(1, stakeCredential));
      const cslAddress = rewardAddress.to_address();
      expect(cslAddress.to_bech32()).toBe(rewardAccount);
    ...
    it('poolRegistration', () => {
      const certificate = coreToCsl.certificate.poolRegistration(scope, poolParameters);
      ...
      const poolRewardAccountCslAddress = scope.manage(poolRewardAccount.to_address());
      ...
      const stakeCredential = scope.manage(CSL.StakeCredential.from_keyhash(owner));
      const rewardAddress = scope.manage(CSL.RewardAddress.new(1, stakeCredential));
      const cslAddress = scope.manage(rewardAddress.to_address());
      ...
      expect(cslAddress.to_bech32()).toBe(ownerRewardAccount);
      ...
      expect(poolRewardAccountCslAddress.to_bech32('stake')).toBe(rewardAccount);
      expect(metadataUrl.url()).toBe(metadataJson.url);
      expect(Buffer.from(metadataHash.to_bytes()).toString('hex')).toBe(metadataJson.hash);
    });

    it('poolRetirement', () => {
      const retirement = scope.manage(coreToCsl.certificate.poolRetirement(scope, poolId, 1000).as_pool_retirement())!;
  ...
  describe('cslToCore.certificate', () => {
    it('stakeKeyRegistration', () => {
      const cert = cslToCore.certificate.createCertificate(
        coreToCsl.certificate.stakeKeyRegistration(scope, stakeKeyHash)
    ...
    it('stakeKeyDeregistration', () => {
      const cert = cslToCore.certificate.createCertificate(
        coreToCsl.certificate.stakeKeyDeregistration(scope, stakeKeyHash)
      ) as Cardano.StakeAddressCertificate;
    ...
    it('stakeDelegation', () => {
      const cert = cslToCore.certificate.createCertificate(
        coreToCsl.certificate.stakeDelegation(scope, stakeKeyHash, poolId)
    ...
    it('poolRegistration', () => {
      const cert = cslToCore.certificate.createCertificate(
        coreToCsl.certificate.poolRegistration(scope, poolParameters)
    ...
    it('poolRetirement', () => {
      const cert = cslToCore.certificate.createCertificate(
        coreToCsl.certificate.poolRetirement(scope, poolId, 1000)

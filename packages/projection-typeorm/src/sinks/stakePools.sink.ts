...
const insertPoolUpdates = async ({ stakePools: { updates }, queryRunner, blockEntity }: Event) => {
  if (updates.length === 0) return;
...
const insertPoolRetirements = async ({ stakePools: { retirements }, queryRunner, blockEntity }: Event) => {

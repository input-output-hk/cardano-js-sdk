...
describe('storeAssets', () => {
  ...
  it('increments asset supply on 2nd mint, decrements when 2st mint block is rolled back', async () => {
    ...
    const totalSupplyAfter2ndMint = secondMintEvent.mintedAssetTotalSupplies[assetIdThatWasMintedTwice]!;
    expect(totalSupplyAfter2ndMint).not.toBeUndefined();
    const rollbackEvent = await projectTilFirst(
      (evt) =>
        evt.eventType === ChainSyncEventType.RollBackward && evt.block.header.hash === secondMintEvent.block.header.hash
    );
    expect(secondMintEvent.mint.length).toBeGreaterThan(0);
    expect(rollbackEvent.mintedAssetTotalSupplies[assetIdThatWasMintedTwice]).toBeLessThan(totalSupplyAfter2ndMint);

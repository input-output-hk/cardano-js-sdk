...
describe('storeStakePoolRewardsJob', () => {
  it('sends jobs at epoch rollover', async () => {
    ...
    expect(send).toBeCalledWith(
      STAKE_POOL_REWARDS,
      { epochNo: 0 },
      { retryDelay: 30, retryLimit: 1_000_000, singletonKey: '0', slot: 1010 }
    );
    expect(send).toBeCalledWith(
      STAKE_POOL_REWARDS,
      { epochNo: 0 },
      { retryDelay: 30, retryLimit: 1_000_000, singletonKey: '0', slot: 1030 }
    );
    expect(send).toBeCalledWith(
      STAKE_POOL_REWARDS,
      { epochNo: 1 },
      { retryDelay: 30, retryLimit: 1_000_000, singletonKey: '1', slot: 2010 }
    );
  });
});

...
import { SERVICE_DISCOVERY_BACKOFF_FACTOR_DEFAULT, SERVICE_DISCOVERY_BACKOFF_TIMEOUT_DEFAULT } from './Program';
import { URL } from 'url';
  ...
  try {
    const worker = await loadTxWorker({
      options: {
        ...
        serviceDiscoveryBackoffTimeout: env.SERVICE_DISCOVERY_BACKOFF_TIMEOUT
      }
    });
    await worker.start();
    onDeath(async () => {

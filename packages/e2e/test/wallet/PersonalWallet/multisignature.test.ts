...
describe('PersonalWallet/multisignature', () => {
  let aliceWallet: PersonalWallet;
  let bobWallet: PersonalWallet;
  ...
  beforeAll(async () => {
    aliceWallet = (await getWallet({ env, idx: 0, logger, name: 'Alice Wallet', polling: { interval: 50 } })).wallet;
    bobWallet = (await getWallet({ env, idx: 1, logger, name: 'Bob Wallet', polling: { interval: 50 } })).wallet;
  ...
  it('can create a transaction with multiple signatures to mint an asset', async () => {
    ...
    bobSigner = new util.KeyAgentTransactionSigner(bobWallet.keyAgent, bobPolicyBuilder.getDerivationPath());

...
export class CardanoWalletFaucetProvider implements FaucetProvider {
  ...
  public async multiRequest(
    ...
    const faucetWallet = await this._walletServer.getShelleyWallet(this._faucetWalletId);
  ...
  public async start(): Promise<void> {
    ...
    const axiosResponse = await this._walletServer.walletsApi.postWallet(walletInfo).catch((error) => {
      ...
      if (error.response.status === HTTP_ERROR_CODE_IN_CONFLICT) {
        ...
        this._faucetWalletId = error.response.data.message.match(/(?<=: ).*(?= H)/g)[0];
      } else {
        throw error.response.data;
      }
    });

    if (axiosResponse) this._faucetWalletId = axiosResponse.data.id;
  }
